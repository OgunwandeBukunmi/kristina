  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        ...
        const res = await fetch("/api/post/resources");
        ...
        let externalPosts = [];
        try {
          const subRes = await fetch(`/api/substack?page=${currentPage}&limit=5&space=resources`);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {posts.map((post) => {
            const href = `/resources/${encodeURIComponent(post._id)}`;
